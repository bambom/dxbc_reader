...
class INPUT {
	  dcl_input posOS.xyzw  	// POSITION  位置
	  dcl_input UV.xy		// TEXCOORD0 UV
	  dcl_input UV1.xyzw		// TEXCOORD1 
	  dcl_input vcolor.xyzw		// COLOR  	定点色
}
class OUT {
	  dcl_output_siv posHClip.xyzw, position //positionHCS 
      dcl_output o1.xyzw
...
void main(INPUT in) {
        r0.xyzw = posOS.yyyy * cb2[1].xyzw; //cb2 是m 矩阵unity_ObjectToWorld
        r0.xyzw = cb2[0].xyzw * posOS.xxxx + r0.xyzw
        r0.xyzw = cb2[2].xyzw * posOS.zzzz + r0.xyzw;
		...
		// vcolor.y == 0.28235 , 0.8549 , 0   , 1   , 0.14118 , 0.71373 , 0.56863 
		// 		== 2.47645 , 6.4843 , 0.5 , 7.5 , 1.48826 , 5.49    , 4.48
        r0.w = vcolor.y * r0.w + 0.5; // v.3 是顶点色的话 ，
		//      == 2	   , 6		, 0    ,7    , 1      , 5       ,  4
        ...
        cloudUV.zw = UV.xxxy * _CloudCurTiling.xxxx + r1.xxxy; //噪点uv
		...
		//smoothstep(UV1.w, 1.0, UV1.y / max(UV1.x, 0.0) * cb0[27].w);
	// 已知cb0[27].w实际值为1.0
	// 已知UV1.x实际值总大于零
	// 故原smoothstep可简化为
		// smoothstep(UV1.w, 1.0, UV1.y / UV1.x);
		{
			r1.y = 1.0 - UV1.w;
